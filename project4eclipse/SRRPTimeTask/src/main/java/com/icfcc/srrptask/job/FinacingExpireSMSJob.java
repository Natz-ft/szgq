...
public class FinacingExpireSMSJob  extends JobObkect {
	// job执行方法
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		String msg = "";
		try {
			Map<String, String> map = platformConfigService.findByConfigNames();
			String NoFocusCount = map.get("noFocusTime").toString();
			String NoTalksCount =  map.get("noTalksTime").toString();
			String NoInvestCount =  map.get("noInvestInfoTime").toString();
			jobLog.setJobId(jobId);
			jobLog.setExeResult(executeResult);// 就绪
			jobLog.setJobName(jobName);// 任务名称
			jobLog.setStartTime(new Date());// 记录任务开始时间
            // 每天检查20内天无机构关注的需求，发送短信			
			financingInfoDetailService.queryFinancingNoFocus(NoFocusCount);
//			//每天检查关注需求后有效期为20天用户逾期未操作 自动关闭需求，发送短信
			financingEventService.findFinacingEventNoTalks(NoTalksCount);
//			//每天检查未投资的融资事件是否超过有限期30天，60，70 //发送短信通知
			financingEventService.queryFinancingNoInvest(NoInvestCount);
			callMakeHtmlControl(smscallURL, jobName, jobId);
			executeResult = SRRPConstant.EXECUTSUCC;
			msg = "完成为关注的需求超过有限期发送短信通知";
		} catch (Exception e) {
