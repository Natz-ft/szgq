		...
		<div action="" id="queryFormId">
		    <div class="modal-body selectmodal wrap-s" >
				<table class="grayTable" data-prompt-position="topLeft">
					<tbody>
						<tr>
							<th align="right" class="thStyle"><span>用户账号:</span></th> 
							<td>
								<input type="text" name="search_LIKE_username" value="${page.paramsMap.search_LIKE_username }" onkeydown="if(event.keyCode==13){event.keyCode=0;return false}" class="textinput" />
							</td>
						</tr>
						<tr>
							<th align="right" class="thStyle"><span>用户姓名:</span></th> 
							<td>
								<input type="text" name="search_LIKE_nickname" value="${page.paramsMap.search_LIKE_nickname }" onkeydown="if(event.keyCode==13){event.keyCode=0;return false}" class="textinput" />
							</td>
						</tr>
						<tr>
							<th align="right" class="thStyle"><span>用户类别:</span></th> 
							<td>
								<select name="search_EQ_org" class="wrap-select">
									<option value=""  selected="selected">请选择</option>
									<c:forEach var="org" items="${sessionScope.org}">
										<option value="${org.key}" <c:if test="${org.key==page.paramsMap.search_EQ_org}">
											selected="selected"
