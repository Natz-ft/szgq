	                  ...
	                  <div class="layui-form-item">
		                ...
		                <div class="layui-form-item">
		                    <label class="layui-form-label" style="width:134px;"><span class="star">*</span>注册时间：</label>
		                     <div class="layui-input-block">
		                       <input type="text" name="registeTime" value='${subac.registeTime}' autocomplete="off"  readonly="readonly" class="form-input input-insert-data" style="margin-top:4px;">
		                   </div>
		                </div>
		                  <div class="layui-form-item">
		                    ...
		                    <div class="input-insert-data" style="margin-left: 134px;">
		                        <select name="subacType" id="subacType" lay-verify="required" lay-search="" lay-filter="selectFilter" style="margin-top:4px;">
		                    		...
		                    		<option value='${d.dicCode}' <c:if test ='${d.dicCode eq subac.subacType}'>selected</c:if>>${d.dicName}</option>
<%-- 		                                <option value='${d.dicCode}'>${d.dicName}</option> --%>
		                ...
		                <div id="flagId" class="layui-form-item" >
					      ...
					      <div class="layui-input-block" style="margin-left: 110px;">
                                ...
                                <select id="trusteeshipRadio" name="trusteeshipRadio" lay-filter="trusteeshipRadio">
					                ...
					                <input id="trusteeshipid"  name="trusteeship" lay-verify="trusteeship" placeholder="请输入" 
            ...
            <div class="form-right">
               <div class="layui-form-item">
		        ...
		        <div class="layui-input-block">
                  <input type="text" name="userName" value='${user.username}' readonly="readonly" autocomplete="off" class="form-input input-insert-data" maxlength="18" style="margin-top:4px;">
		        </div>
		       </div>
...
<script>
    ...
    $(function (){
   	 ...
   	 var creditAuthorizationPath='${subac.creditAuthorizationPath}';
   	 var licensePathVal = '${subac.licensePath}';
    	//指定允许上传的文件类型 上传征信授权书
...
//                    , exts: 'jpg|jpeg|png|bmp|gif|txt|TXT|xls|XLS|xlsx|docx|doc|pdf'
    	...
    	function getCountyChildren(parent){
    		...
    		var areaCounty="${subac.areaCounty}";
    		for(var i=0;i<childrenData.length;i++){
    	        if(childrenData[i].parentcode==parent){
		...
		layui.use('form', function(){
			  ...
			  if("" !="${subac.areaProvince}"){
					var options="${subac.areaProvince}";
					console.log(options);
					var childrenDataStr="";
		...
		form.on('select(areaCity)', function(data){
		      ...
		      form.verify({
		            ...
		            trusteeship:function(){
		            	var trusteeshipRadioVal = $("#trusteeshipRadio").val();
		            	var trusteeshipVal = $("#trusteeshipid").val();
		            	if(trusteeshipRadioVal=="0"){
		            		if(trusteeshipVal=="无" || trusteeshipVal=="" || trusteeshipVal==null || trusteeshipVal==undefined){
		            			return"请输入托管机构名称";
		            		}
		            	}
			  ...
			  form.on('submit(investor)', function(data){
				  ...
				  $.ajax({
					   type : "post",
					  async : false,
						url : "/creditplatformweb/inverstorUser/updateUser",
		...
		$("#flagId div dl dd").click(function () {
	        var options = $("#trusteeshipRadio option:selected").val();
	        var trusteeshipVal = $("#trusteeshipFlag").val();
	        if (options == '1') {
	            $("input[name=trusteeship]").prop('value','无');
	            $("input[name=trusteeship]").prop('type', 'hidden');
	        } else {
	            $("input[name=trusteeship]").removeAttr("type");
	            if(trusteeshipVal=='无'){
	            	$("input[name=trusteeship]").prop('value','');
	            }else{
	            	$("input[name=trusteeship]").prop('value',trusteeshipVal);
	            }
