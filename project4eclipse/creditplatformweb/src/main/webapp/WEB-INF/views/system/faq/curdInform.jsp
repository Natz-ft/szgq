		...
		<thead >	
              <tr>
               <td class="tr_td_operate" colspan="8" align="left" >
	                ...
	                <!-- 这里还需要更改后台返回页面 -->
	                <button type="button" onclick="delete_button('id','/faq/delInform?id=');" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only ui-state-hover button6"  role="button" aria-disabled="false" id="delete_config" >删除</span></button>
		...
		<tbody >
			<c:forEach var="faq" items="${page.list}">
				<tr >
					...
					<td>
					<input name="id" autocomplete="off"   value="${faq['id'] }" class="inputCheckBox" type="checkbox"  /></td>
...
<form action="curdFaq" id="list_form_id" method="post" name="list_form_name" onkeydown= "if(event.keycode==13)return   false;">  
	<c:import url="/WEB-INF/views/common/pagination.jsp" />
	<!-- 修改为自己查询页面的路径 -->
	<c:import url="/WEB-INF/views/system/faq/query_inform.jsp" />
</form>
...
function create_page() {
	layer.open({
	  	  ...
	  	  shade: 0,
	  	  area: ['70%', '85%'],
	  	  content: '/creditplatformweb/faq/createPageInform' //iframe的url
	  	}); 
...
function showDetail(id) {
	layer.open({
	  	  ...
	  	  shade: 0,
	  	  area: ['70%', '85%'],
	  	  content:  "/creditplatformweb/faq/detailInform?id=" + id //iframe的url
	  	}); 
...
function updateDetail(id) {
	layer.open({
	  	  ...
	  	  shade: 0,
	  	  area: ['70%', '85%'],
	  	  content:  "/creditplatformweb/faq/InformUpdate?id=" + id //iframe的url
	  	}); 
