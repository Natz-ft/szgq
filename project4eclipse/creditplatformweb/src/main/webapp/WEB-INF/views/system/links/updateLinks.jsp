...
<script src="${pageContext.request.contextPath}/static/js/jquery-1.9.1.min.js" type="text/javascript"></script>
 ...
 $(function () {
     ...
     var filepath="${links.logo}";
     var arr = filepath.split('/');
     document.getElementById("showPic").innerText = arr[arr.length-1];
     var id=${links.id};
     //普通图片上传
    var upload = upload.render({
       elem: '#uplodBtn'
       ,url: '/creditplatformweb/news/upload/',
       data: {type: '/links/' ,id: id}
     ,before: function(obj){
	        //预读本地文件示例，不支持ie8
     ...
     $('#showPic').on('click', function () {
    	 ...
    	 var  content= '<div style="text-align: center;"><img src="'+filepath+'" onerror="'+ error +'" height="300px;" width="350px;"/></div>';layer.open({
             type: 1,
             title: false, //不显示标题栏
             closeBtn: false,
             //area: '300px;',
   ...
   //监听审核按钮
     $("#uploadbtnListAction").on('click', function () {
    	 ...
    	 $.ajax({
             type: "post",
             async: false,
             url: "/creditplatformweb/links/update",
             data: {
                 "links": JSON.stringify(dataJson)
             },// 你的formid
...
<div class="boxWrap">
    <p class="boxTitle"><span>编辑友情链接</span></p>
    <div class="formWrap">
            <div class="layui-form-item inputTitle">
                <label class="layui-form-label"><span class="star">*</span>友情链接名称：</label>
                <div class="layui-input-block" style="padding: 5px 0px;">
                    <input type="text" id="name" name="name" value="${links.name}"  autocomplete="off" class="layui-input">
                </div>
            </div>
            <div class="layui-form-item inputTitle">
                <label class="layui-form-label"><span class="star">*</span>友情链接地址：</label>
                <div class="layui-input-block" style="padding: 5px 0px;">
                    <input type="text" id="url" name="url" value="${links.url}"  autocomplete="off" class="layui-input">
                </div>
            </div>
<!-- 左右两列左侧容器 -->
               <div class="layui-form-item">
                 <label class="layui-form-label">友情链接logo：</label>
