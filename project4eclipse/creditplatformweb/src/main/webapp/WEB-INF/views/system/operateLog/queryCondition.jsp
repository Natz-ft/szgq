		...
		<div action="" id="queryFormId">
		    <div class="modal-body selectmodal wrap-xs" >
				<table class="grayTable" data-prompt-position="topLeft">
					<tbody>
						<tr>
							<th align="right" class="thStyle"><span>用户名:</span></th> 
							<td  colspan="3">
								<input type="text" name="search_LIKE_userName" value="${page.paramsMap.search_LIKE_userName}"  onkeydown="if(event.keyCode==13){event.keyCode=0;return false}" />
							</td>
						</tr>
				<tr>
							...
							<td>
								<input type="text" name="search_GTE_createTime_DATE"  onfocus=" WdatePicker({dateFmt:'yyyy-MM-dd'})" value="${GTE_createTime}" onkeydown="if(event.keyCode==13){event.keyCode=0;return false}" /> 至
							    <input type="text" name="search_LTE_createTime_DATE"  onfocus=" WdatePicker({dateFmt:'yyyy-MM-dd'})"  value="${LTE_createTime}" onkeydown="if(event.keyCode==13){event.keyCode=0;return false}" /> 
							</td>
						</tr> 
					</tbody>					
				</table>
