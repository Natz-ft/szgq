...
<!-- 	           	    <button type="button" onclick="create_button('create_dialog_id');" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only ui-state-hover button6"  role="button" aria-disabled="false" id="save_config" >新建</button>
 -->	                <!-- 开发人员根据更新服务的URL ，将URL 作为参数传入 例如本案例的更新URL为‘/systemConfig/getSystemConfigById.do’，则在update_button方法中，出入该URL -->
	                <!-- 这里还需要更改后台返回页面 -->
	                <button type="button" onclick="delete_button('id','/partner/delPartner?id=');" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only ui-state-hover button6"  role="button" aria-disabled="false" id="delete_config" >删除</span></button>
		...
		<tbody >
			<c:forEach var="partner" items="${page.list}">
				<tr >
					<!-- 开发人员根据业务需求，自定义表格内容字段 开始 -->
					<td>
					<input autocomplete="off"   name="id" value="${partner['id'] }" class="inputCheckBox" type="checkbox"  /></td>
					<td>${partner['name'] }</td>
					<td><img src="${partner.logo }" onerror="this.src='/creditplatformweb/static/image/uploadImg.jpg' " height="30px" width="40px" /></td>
					<td>${partner.link}</td>
					<td><fmt:formatDate value="${partner['createTime'] }" pattern='yyyy-MM-dd HH:mm:ss'/></td>
					<td><a href="#" onClick="updateDetail('${partner['id'] }');">编辑</a></td>
					<!-- 开发人员根据业务需求，自定义表格内容字段 结束 -->
				</tr>
			</c:forEach>
		</tbody>
</table>
<form action="curdPartner" id="list_form_id" method="post" name="list_form_name" onkeydown= "if(event.keycode==13)return   false;">  
	<c:import url="/WEB-INF/views/common/pagination.jsp" />
	<!-- 修改为自己查询页面的路径 -->
	<c:import url="/WEB-INF/views/system/partner/query_condition.jsp" />
</form>
<!-- 提示页面应该作为公共页面，没必要每个人都拷贝 -->
<c:import url="/WEB-INF/views/system/partner/show_partner.jsp" /> 
</div>
...
// 	  function callback() {
//       }
//       layerOpen("demoId", "930", "600", "", "/creditplatformweb/partner/createPage",2, callback);
      layer.open({
    	  type: 2,
    	  title: '新增',
    	  shadeClose: true,
    	  ...
    	  shade: 0,
    	  area: ['70%', '85%'],
    	  content: '/creditplatformweb/partner/createPage' //iframe的url
    	});  
...
function updateDetail(id) {
	layer.open({
  	  ...
  	  shade: 0,
  	  area: ['70%', '85%'],
  	  content: "/creditplatformweb/partner/updatePartner?id=" + id//iframe的url
  	});  
