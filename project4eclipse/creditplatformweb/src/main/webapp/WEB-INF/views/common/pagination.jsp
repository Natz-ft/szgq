...
#page_table,#page_table tr,#page_table td {
       border: none;
}
#page_table td {
    padding-left: 2px;
...
<table width="100%" id="page_table">
	<tr>
		<td align="center" valign="middle"><img src="${ctx}/static/image/list-06.jpg" /> 
			共
		...
		<c:if test="${!empty page.curPage}">
		 ...
		 <c:if test="${empty page.recordCnt}">
		<input  type="hidden" id="recordCnt" name="recordCnt" value="0">
		</c:if>
		<c:if test="${!empty page.recordCnt}">
				<input  type="hidden" id="recordCnt" name="recordCnt" value="${page.recordCnt}">
		</c:if>
