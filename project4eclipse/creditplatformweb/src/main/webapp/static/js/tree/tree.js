...
acTreeItem.prototype.toString = function (nItem, nItemCount) {
	...
	if(i<1){
		...
		"" +
		"<img id=\"" + this.id + "-icon\" class=\"ac-tree-icon\" src=\"" + ((acTreeHandler.behavior == 'classic' && this.open)?this.icon:this.icon) + "\" onclick=\"acTreeHandler.toggle(document.getElementById('"+this.id+"'));\">" +
