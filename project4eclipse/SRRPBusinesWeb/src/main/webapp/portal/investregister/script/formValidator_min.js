...
(function ($) {
    $.formValidator = {
        sustainType: function (a, b) {
            ...
            var b = a.id, c = $("#" + b).get(0), d = a.isvalid, e = a.setting, f = "", g = "",
                h = $("#" + b).get(0).settings, i = $("body").data(h[0].validatorgroup);
            if (!d) {
            ...
            } else f = $.formValidator.isEmpty(b) ? e.onempty : e.oncorrect, $.formValidator.setTipState(c, "onCorrect", f);
            return f
            ...
            switch (sType) {
                ...
                case"file":
                    ...
                    if (f.wideword) for (var g = 0; g < e.length; g++) d = d + (e.charCodeAt(g) >= 19968 && e.charCodeAt(g) <= 40869 ? 2 : 1); else d = e.length;
                    break;
                case"checkbox":
                case"radio":
            ...
            if (b) b = i.onsuccess(), i.submitonce && $(":submit,:button").attr("disabled", !0); else {
                var j = $("#" + e).get(0);
            ...
            if (c.size() == 0 && e[0].empty) a.setting = e[0], a.isvalid = !0, $.formValidator.showMessage(a), f.isvalid = !0; else {
                var h = $("body").data(e[0].validatorgroup), i = $.param($(h.ajaxobjectids).serializeArray());
                i = "clientid=" + b + (i.length > 0 ? "&" + i : ""), g = g + (g.indexOf("?") > -1 ? "&" + i : "?" + i), $.ajax({
                    type: f.type,
                    url: g,
                    cache: f.cache,
                    data: f.data,
            ...
            var b = a.id, c = a.setting, d = $("#" + b), e = d.get(0), f = d.val(), g = e.type,
                h = $.formValidator.getLength(b), i = c.empty, j = !1;
            switch (g) {
                case"text":
                case"hidden":
                case"password":
                ...
                case"radio":
                    ...
                    var l = c.type;
                    if (l == "size") j || (k = !0), k && (f = h); else if (l == "date" || l == "datetime") {
                        var m = !1;
                        l == "date" && (k = isDate(f)), l == "datetime" && (k = isDate(f)), k && (f = new Date(f), c.min = new Date(c.min), c.max = new Date(c.max))
                    } else stype = typeof c.min, stype == "number" && (f = (new Number(f)).valueOf(), isNaN(f) || (k = !0)), stype == "string" && (k = !0);
            ...
            if (f == "number") if (!isNaN(curvalue) && !isNaN(ls_data)) curvalue = parseFloat(curvalue), ls_data = parseFloat(ls_data); else return;
            if (f == "date" || f == "datetime") {
                var g = !1;
                f == "date" && (g = isDate(curvalue) && isDate(ls_data)), f == "datetime" && (g = isDateTime(curvalue) && isDateTime(ls_data));
                if (g) curvalue = new Date(curvalue), ls_data = new Date(ls_data); else return
            }
            switch (c.operateor) {
                case"=":
        ...
        return this.each(function (a) {
            ...
            if (c.autotip) if (!c.tidymode) {
                if ($("body [id=" + f + "]").length == 0) {
                    var g = e.relativeid ? e.relativeid : this.id, h = $("#" + g).position(), i = h.top,
                        j = $("#" + g).width() + h.left;
                    $("<div class='formValidateTip'></div>").appendTo($("body")).css({
                        left: j + "px",
                        top: i + "px"
                    }).prepend($('<div id="' + f + '"></div>').css(e.tipcss)), b.afterid = g
                }
            } else d.showTooltips();
            ...
            n && d.val(n), l == "input" || l == "textarea" ? (d.focus(function () {
                ...
                if (d != null) if (d.ajax >= 0) $.formValidator.showAjaxMessage(d); else {
                    var e = $.formValidator.showMessage(d);
                    if (!d.isvalid) {
                        var f = b.automodify && (this.type == "text" || this.type == "textarea" || this.type == "file");
                        if (f && !c.alertmessage) alert(e), $(this).val(this.validoldvalue), $.formValidator.setTipState(this, "onShow", b.onshow); else if (c.forcevalid || b.forcevalid) alert(e), this.focus()
                    }
                }
            })) : l == "select" && d.bind({
                focus: function () {
