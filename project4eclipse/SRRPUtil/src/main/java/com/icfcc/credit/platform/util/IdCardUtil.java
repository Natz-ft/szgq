...
public class IdCardUtil {
	...
	public static void main(String[] args) {
		String idcard= "371326198904223719";
		// String id = "123451234512345";
		// System.out.println("a:" + taiwanIdpassValidator(id));
//		String idpass = "1234567890(11)";
		System.out.println(checkCardStrict(idcard));
	}
	...
	private static boolean isDate(String date) {
		...
		if (!StringUtils.isNumeric(month) || Integer.parseInt(month) > 12 || Integer.parseInt(month) < 01)
			return false;
		if (Integer.parseInt(day) > getMaxDay(Integer.parseInt(year), Integer.parseInt(month))
				|| Integer.parseInt(day) < 01)
			return false;
		return true;
	...
	public static boolean checkCardStrict(String idcard) {
		...
		String[] idcard_array = new String[18];
		int S, Y;
		String M;
		String JYM;
		...
		if (idcard.length() > 0 && idcard != null) {
			...
			if (isDate(date)) {
				...
				if (Integer.parseInt(M) == Integer.parseInt(idcard_array[17])) {
					return true;
				} else {
					return false;
				}
