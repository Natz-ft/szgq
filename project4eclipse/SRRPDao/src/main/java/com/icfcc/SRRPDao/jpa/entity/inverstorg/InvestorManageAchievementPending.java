...
@Data
@Entity
@Table(name = "rp_investor_manage_achievement_pending")
public class InvestorManageAchievementPending implements Serializable{

	
	private static final long serialVersionUID = -4340008882453742401L;
	...
	public void setFinanceStageDicCode(String financeStageDicCode) {
		...
		if (financeStageDicCode != null) {
			...
			for (int i = 0; i < values.length; i++) {
				String Trade = null;
				Trade = RedisGetValue.getCodeByValue(SRRPConstant.DD_INVESTMENT, values[i]);
				if (Trade != null) {
					if (sb.length() > 0) {
						sb.append(",").append(Trade);
					} else {
	...
	public void setFinanceTradeDicCode(String financeTradeDicCode) {
		...
		if (financeTradeDicCode != null && !"".equals(financeTradeDicCode)) {
			...
			for (int i = 0; i < values.length; i++) {
				...
				Trade = RedisGetValue.getCodeByValue(SRRPConstant.DD_INDUSTRY, values[i]);
				if (Trade != null) {
					if (sb.length() > 0) {
						sb.append(",").append(Trade);
					} else {
					...
					Trade = RedisGetValue.getCodeByValue(SRRPConstant.DD_INDUSTRY_2, values[i]);
					if (Trade != null) {
						if (sb.length() > 0) {
							sb.append(",").append(Trade);
						} else {
	...
	public void setFoundTypeDicCode(String foundTypeDicCode) {
		...
		if (foundTypeDicCode != null  && !"".equals(foundTypeDicCode)) {
			Trade = RedisGetValue.getCodeByValue(SRRPConstant.DD_SUBACTYPE, foundTypeDicCode);
			this.fundType = Trade;
		}
	...
	public void setCiCurrencyCode(String ciCurrencyCode) {
		...
		if (ciCurrencyCode != null && !"".equals(ciCurrencyCode)) {
			...
			Trade = RedisGetValue.getCodeByValue(SRRPConstant.DD_CURRENCY, ciCurrencyCode);
	...
	public void setCurrencyDicnameCode(String currencyDicnameCode) {
		...
		if (currencyDicnameCode != null && !"".equals(currencyDicnameCode)) {
			...
			Trade = RedisGetValue.getCodeByValue(SRRPConstant.DD_CURRENCY, currencyDicnameCode);
			this.currency = Trade;
		}
	}
