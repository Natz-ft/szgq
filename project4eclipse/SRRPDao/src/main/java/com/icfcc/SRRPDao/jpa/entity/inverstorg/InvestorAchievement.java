...
@Data
@Entity
@Table(name = "rp_investor_achievement")
public class InvestorAchievement implements Serializable {
	...
	public void setInvestedEnterpriseIndustryCode(String investedEnterpriseIndustryCode) {
		String Trade=null;
		if (investedEnterpriseIndustryCode != null && !"".equals(investedEnterpriseIndustryCode)) {
			this.investedEnterpriseIndustry = RedisGetValue.getCodeByValue(SRRPConstant.DD_INDUSTRY,
					investedEnterpriseIndustryCode.trim());
			Trade = RedisGetValue.getCodeByValue(SRRPConstant.DD_INDUSTRY, investedEnterpriseIndustryCode.trim());
			if (Trade != null && !"".equals(Trade)) {
				this.investedEnterpriseIndustry =Trade;
			}else{
				Trade = RedisGetValue.getCodeByValue(SRRPConstant.DD_INDUSTRY_2, investedEnterpriseIndustryCode.trim());
				if (Trade != null) {
					this.investedEnterpriseIndustry =Trade;
				}	
			}
	...
	public void setRoundOfInvestmentCode(String roundOfInvestmentCode) {
		if (roundOfInvestmentCode != null && !"".equals(roundOfInvestmentCode)) {
			this.roundOfInvestment = RedisGetValue.getCodeByValue(SRRPConstant.DD_FINACINGTURN, roundOfInvestmentCode.trim());
		}
	}
	...
	public void setInvestmentStageCode(String investmentStageCode) {
		if (investmentStageCode != null && !"".equals(investmentStageCode)) {
			this.investmentStage = RedisGetValue.getCodeByValue(SRRPConstant.DD_INVESTMENT, investmentStageCode.trim());
		}
	}
	...
	public void setInvestedEnterpriseAreaCode(String investedEnterpriseAreaCode) {
		if (investedEnterpriseAreaCode != null  && !"".equals(investedEnterpriseAreaCode)) {
			this.investedEnterpriseArea = RedisGetValue.getCodeByValueArea(SRRPConstant.DD_AREA,
					investedEnterpriseAreaCode.trim());
		}
	}
	...
	public String getInvestedEnterpriseIndustryStr() {
		if (investedEnterpriseIndustry != null) {
			if (investedEnterpriseIndustry.length() > 2) {
				investedEnterpriseIndustryStr = RedisGetValue.getValueByCode(SRRPConstant.DD_INDUSTRY_2,
						investedEnterpriseIndustry);
			} else {
				investedEnterpriseIndustryStr = RedisGetValue.getValueByCode(SRRPConstant.DD_INDUSTRY,
						investedEnterpriseIndustry);
			}
	...
	public String getIaCurrencyDicname() {
		return RedisGetValue.getValueByCode(SRRPConstant.DD_CURRENCY, this.iaCurrency);
	}
	...
	public String getEcCurrencyDicname() {
		return RedisGetValue.getValueByCode(SRRPConstant.DD_CURRENCY, this.ecCurrency);
	}
	...
	public String getIaCurrencyStr() {
		...
		if (this.investmentAmount == 0) {
			...
			iaCurrencyStr = DigitFormatUtil.digitFormat(this.investmentAmount).toString() + " "
					+ this.getIaCurrencyDicname();
		}
	...
	public String getEcCurrencyStr() {
		...
		if (this.enterpriseCapital == 0) {
			...
			ecCurrencyStr = DigitFormatUtil.digitFormat(this.enterpriseCapital).toString() + " "
					+ this.getEcCurrencyDicname();
		}
	...
	public void setIaCurrencyCode(String iaCurrencyCode) {
		...
		if (iaCurrencyCode != null && !"".equals(iaCurrencyCode)) {
			...
			Trade = RedisGetValue.getCodeByValue(SRRPConstant.DD_CURRENCY, iaCurrencyCode.trim());
	...
	public void setEcCurrencyCode(String ecCurrencyCode) {
		...
		if (ecCurrencyCode != null && !"".equals(ecCurrencyCode)) {
			...
			Trade = RedisGetValue.getCodeByValue(SRRPConstant.DD_CURRENCY, ecCurrencyCode.trim());
	...
	public void setSignOutCode(String signOutCode) {
		String Trade = "";
			Trade = RedisGetValue.getCodeByValue(SRRPConstant.DD_RADIO, signOutCode.trim());
			this.signOut = Trade;
	}
